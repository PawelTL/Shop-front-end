...
function Store({ products, loadProducts }) {
  ...
  return (
    <motion.div
      className='w-screen bg-slate-100 mt-[9vh] pb-[9vh] min-h-[91vh]'
      initial={{ width: 0 }}
      animate={{ width: "100vw" }}
      exit={{ x: window.innerWidth, transition: { duration: 0.1,} }}>

      <div className='relative flex items-center'>
        <h1 className='py-3 pl-5 pr-3 text-2xl font-semibold md:px-10 md:text-5xl'>Our Products {'>'}</h1>
        <button type='button' onClick={() => setActiveDropdown(!activeDropdown)} className='bg-slate-300 min-w-[80px] min-h-[40px] w-[8vw] h-[2.5vw] text-2xl md:text-4xl rounded-xl border-2 border-slate-400 font-bold'>All</button>
        {activeDropdown ? <motion.ul initial={{scaleY: 0}} animate={{scaleY: 1}} className=' origin-top absolute top-[100%] left-[24%] z-10 flex flex-col w-[220px] bg-slate-100 border-4 border-slate-300 rounded-md text-xl justify-evenly'>
      ...
      <div className='flex flex-wrap justify-center w-full gap-6 px-10 pb-4'>
        {list.length > 0 ?
